...
describe('fact page/session bindings', () => {
  ...
  test('missing, malformed, or out-of-scope remote auth never queries binding storage', async () => {
    ...
    const denied = [
      ...
      { remote: true, auth: { clientName: 'brain-router-owner-0123456789ab', scopes: ['read'], surfaceSetBy: 'operator' } },
      { remote: true, auth: { scopes: ['read'], fleetGrant: 'fleet_router' } },
      { remote: true, auth: { scopes: ['read'], fleetGrant: 'fleet_router', fleetGrantVersion: 1, fleetGrantSetBy: 'operator' } },
  ...
  test('authenticated fleet-router get_page receives private binding; ordinary remote gets no oracle', async () => {
    const remoteContext = (clientName: string, authorized = false) => ({
      engine,
      config: {},
      logger: { info() {}, warn() {}, error() {}, debug() {} },
      dryRun: false,
    ...
    try {
      const fleetPage = await operationsByName.get_page.handler(
        remoteContext('brain-router-owner-0123456789ab', true),
        { slug: entityA },
      ) as Record<string, unknown>;
      expect(fleetPage.fact_bindings).toEqual([{
        fact_id: expect.any(Number),
        capture_page_slug: captureA,
        hermes_session_ref: sessionA,
        matched_via: 'entity_fence',
      }]);
      expect(Object.keys((fleetPage.fact_bindings as Array<Record<string, unknown>>)[0]).sort()).toEqual([
      ...
      expect(JSON.stringify(ordinaryPage)).not.toContain(captureA);
      expect(JSON.stringify(ordinaryPage)).not.toContain(sessionA);
      expect(bindingSqlCalls).toBe(1);

      const selfNamedDcrPage = await operationsByName.get_page.handler(
        remoteContext('brain-router-owner-0123456789ab'),
        { slug: entityA },
      ) as Record<string, unknown>;
      ...
      engine.executeRaw = (async (sql: string, params?: unknown[]) => {
        if (sql.includes('WITH refs(source_id, slug)')) throw new Error('fact-binding storage unavailable');
        return originalExecuteRaw(sql, params);
      }) as typeof engine.executeRaw;
      const degraded = await operationsByName.get_page.handler(
        remoteContext('brain-router-owner-0123456789ab', true),
        { slug: entityA },
      ) as Record<string, unknown>;
      expect(degraded.fact_bindings).toBeUndefined();
      expect(JSON.stringify(degraded)).not.toContain(captureA);
      expect(JSON.stringify(degraded)).not.toContain(sessionA);
    } finally {
