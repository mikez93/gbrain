...
describe('mcp:put_page fact capture lineage', () => {
  ...
  test('versioned worker rejects every malformed or stale identity before chat', async () => {
    ...
    const physicalSlugMismatch = rekey({
      ...data,
      sourceSlug: `${captureSlug}-different-physical-page`,
    });
    await expect(invoke(
      physicalSlugMismatch.data,
      { idempotency_key: physicalSlugMismatch.key },
    )).rejects.toThrow(/sourceSlug must exactly match loaded page slug/);
  ...
  test('real worker retries a provider transport failure to one terminal result/fact with zero queue debt', async () => {
    ...
    try {
      ...
      const terminal = await waitFor(async () => {
        ...
        }>(`SELECT id, idempotency_key, data, status, attempts_made, attempts_started, stalled_counter, result,
