...
- **Rerankers**: Voyage `rerank-2.5` hosted (the new-install default; reranking is on in `balanced` and `tokenmax` modes, using the same `VOYAGE_API_KEY` as embeddings), Voyage `rerank-2.5-lite` / `rerank-2.5` through OpenRouter, Cohere and NVIDIA routes through OpenRouter, ZeroEntropy `zerank-2` (deprecated — hosted API ends 2026-09-04), plus the `llama-server-reranker` recipe for local cross-encoder reranking. Hosted setup is in [`docs/integrations/embedding-providers.md`](docs/integrations/embedding-providers.md#openrouter); local setup is in [`docs/ai-providers/llama-server-reranker.md`](docs/ai-providers/llama-server-reranker.md).
